...
void eval(char *cmdline)
{
  ...
  if(builtin_cmd(argv) == 1) /* Build in command was executed */
  {
      return;
  }
  ...
  switch(pid)
  {
    ...
    {
      /* Error */
      unix_error("Error during fork function call...exiting\n");
      break;
    }
    default:
    {
 ...
 * argument.  Return 1 if the user has requested a BG job, other if
...
void waitfg(pid_t pid)
{
  /* Switch status of pid to forground */
...
void sigchld_handler(int sig)
{
    ...
    while((pid = waitpid(-1, &status, WNOHANG | WUNTRACED)) > 0)
    {
      ...
      {
        printf("Job [%d] (%d) terminated unexpectedly\n", pid2jid(pid), pid);
        fflush(stdout);
      }
    }
    return;
